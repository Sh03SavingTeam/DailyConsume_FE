    ...
    return (
        <div className="container sec_container">
            ...
            <div className="rankinfo-container">
                <div 
                    className={`rank1-container ${selectedRank === '거지토순이' ? 'selected' : ''}`}
                    onClick={() => handleRankClick('거지토순이')}
                >
                    <div className="characterImg1">
                        <img src={CharacterImage1} alt="RabbitLv1" className='benefit-rank-image1'/>
                    </div>
                    <div className="rank-name1">
                        거지토순이
                    </div>
                    <div className="rank-range1">
                        점수 0 ~ 19
                    </div>
                </div>
                <div 
                    className={`rank2-container ${selectedRank === '새싹토순이' ? 'selected' : ''}`}
                    onClick={() => handleRankClick('새싹토순이')}
                >
                    <div className="characterImg2">
                        <img src={CharacterImage2} alt="2" className='benefit-rank-image2'/>
                    </div>
                    <div className="rank-name2">
                        새싹토순이
                    </div>
                    <div className="rank-range2">
                        점수 20 ~ 39
                    </div>
                </div>
                <div 
                    className={`rank3-container ${selectedRank === '당근토순이' ? 'selected' : ''}`}
                    onClick={() => handleRankClick('당근토순이')}
                >
                    <div className="characterImg3">
                        <img src={CharacterImage3} alt="3" className='benefit-rank-image3'/>
                    </div>
                    <div className="rank-name3">
                        당근토순이
                    </div>
                    <div className="rank-range3">
                        점수 40 ~ 59
                    </div>
                </div>
                <div 
                    className={`rank4-container ${selectedRank === '부자토순이' ? 'selected' : ''}`}
                    onClick={() => handleRankClick('부자토순이')}
                >
                    <div className="characterImg4">
                        <img src={CharacterImage4} alt="4" className='benefit-rank-image4'/>
                    </div>
                    <div className="rank-name4">
                        부자토순이
                    </div>
                    <div className="rank-range4">
                        점수 60 이상
                    </div>
                </div>
            </div>
            ...
            <div className="footer-info">
                ...
                <span>▪️ 매월 1일, 최근 1개월 등급 점수 기준으로 새로운 회원 등급이 부여됩니다.</span>
                <span>▪️ 새로운 등급은 앱 사용 이벤트에 한하여 부여됩니다.</span>
                <span>▪️ 이벤트란 주간소비 설정금액 달성, 첫 카드등록, 출석체크, 지출비교 를 말합니다.</span>
                <span>▪️ 회원 등급별 혜택과 기준은 내부 사정에 의하여 향후 변경될 수 있습니다.</span>
                <span>▪️ 등급 혜택은 본 페이지에서 '혜택 포인트 받기' 버튼을 통해 받으실 수 있습니다.</span>
