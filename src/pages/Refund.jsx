...
function Refund({ memberId }){
    ...
    const fetchPointAccount = async () => {
        try {
            const response = await axios.get(`http://localhost:9999/mypage/refund/${memberId}`);
        ...
        <div className="refund-container">
            ...
            <div className="refund-info">
                <ul>
                    ...
                    <li>환급 신청 후 포인트는 현금으로 실시간 전환되며,<br/> 이후 취소는 불가합니다.</li>
                    <li>환급된 현금은 회원가입 시 입력한 계좌로 환급됩니다.</li>
                    <li>환급 받을 계좌는 신한은행 계좌만 가능합니다.</li>
                    <li>환급 완료 후 남은 포인트는 마이페이지에서<br/> 확인하실 수 있습니다.</li>
                    <li>현금 환급 신청은 해당 앱에서만 가능합니다.</li>
                </ul>
            </div>
            <div className="refund-check">
                <label>
                    <input type="checkbox" checked={check} onChange={() => setCheck(prevCheck => !prevCheck)} />
                    환급 동의 (전환 시 취소할 수 없습니다.)
                </label>
            </div>
            {warning && <p className="refund-warning">{warning}</p>}
            <div className="refund-button">
                <Link to={check?'/MyPage/Refund/Confirm':'#'} state={{refundAmount:text}} onClick={handleRefundClick}>
