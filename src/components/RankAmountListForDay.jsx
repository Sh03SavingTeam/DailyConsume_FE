...
function RankAmountListForDay({ initialDay, paramMember }) {
  // 상태(state) 선언
  ...
  const fetchWeeklyBudget = async (date, memberId) => {
    try {
      ...
      const response = await axios.get("/api/calendar/payweekly", {
        params: {
          memberId: memberId, // 요청 파라미터로 사용자 ID를 보냄
          year: year, // 요청 파라미터로 선택된 날짜의 년도를 보냄
          month: month, // 요청 파라미터로 선택된 날짜의 월을 보냄
    ...
    <div className="amount-list">
      <div className="detail-item">
        <div className="weekly-budget">
          ...
          <p>
            주간소비금액 :{" "}
            {weeklyBudget ? `${weeklyBudget.설정금액.toLocaleString()}원` : ""}
          </p>
          <p>
            주간소비잔여금액 :{" "}
            {weeklyBudget
...
export default RankAmountListForDay; // AmountListForDay 컴포넌트 export
